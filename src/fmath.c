...
Mat4 mat4_translate(Mat4 mat, Vec4 vec) {
  ...
  result = mul_mat4_mat4(mat, result);
  return result;
}
...
Mat4 mat4_rotate(Mat4 mat, Vec4 axis, f32 radians) {
  Mat4 result   = mat4_make_identity();
  axis          = vec4_normalize(axis);
  f32 sin_theta = sin(radians);
  f32 cos_theta = cos(radians);
  f32 not_cos   = 1 - cos_theta;
  result.data[0][0] = cos_theta + (axis.x*axis.x*not_cos);
  result.data[1][0] = (axis.x*axis.y*not_cos) - (axis.z*sin_theta);
  result.data[2][0] = (axis.x*axis.z*not_cos) + (axis.y*sin_theta);
  result.data[0][1] = (axis.y*axis.x*not_cos) + (axis.z*sin_theta);
  result.data[1][1] = cos_theta + (axis.y*axis.y*not_cos);
  result.data[2][1] = (axis.y*axis.z*not_cos) - (axis.x*sin_theta);
  result.data[0][2] = (axis.z*axis.x*not_cos) - (axis.y*sin_theta);
  result.data[1][2] = (axis.z*axis.x*not_cos) + (axis.x*sin_theta);
  result.data[2][2] = cos_theta + (axis.z*axis.z*not_cos);
  result = mul_mat4_mat4(mat, result);
  return result;
}
